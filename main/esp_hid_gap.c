...
static const ble_uuid16_t s_hid_service_uuid16 = BLE_UUID16_INIT(GATT_SVR_SVC_HID_UUID);

// Tracked so esp_hid_gap_clear_bonds_and_disconnect() can terminate the
// active link without the esp_hidd wrapper layer exposing a conn_handle.
...
esp_err_t esp_hid_ble_gap_adv_init(uint16_t appearance, const char *device_name)
{
    ...
    fields.uuids16 = &s_hid_service_uuid16;
    fields.num_uuids16 = 1;
    fields.uuids16_is_complete = 1;

    /* Initialize the security configuration.
     * iOS's HOGP acceptance needs the HID GATT characteristics themselves to
     * declare an encryption requirement (see CONFIG_BT_NIMBLE_SM_LVL) -- it
     * does NOT need that access to be authenticated (MITM). So we still
     * bond and encrypt, but via plain "Just Works": no display or keypad on
     * this board, so there's no good way to do passkey/numeric-comparison
     * pairing anyway, and it isn't required here. */
...
nimble_hid_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        if (event->connect.status == 0) {
            ...
            // HOGP requires the link to be encrypted before HID reports may be
            // sent; the central doesn't always initiate this on its own, so
            // request it ourselves rather than waiting.
...
{
    bool was_connected = (s_conn_handle != BLE_HS_CONN_HANDLE_NONE);
    if (was_connected) {
        // Termination completes asynchronously; the disconnect event's own
        // handler (ble_hid.c's hidd_event_callback) restarts advertising
        // once it lands, so this function doesn't need to (and starting
        // advertising here too, before that event fires, would race it).
        ble_gap_terminate(s_conn_handle, BLE_ERR_REM_USER_CONN_TERM);
    }

    ble_store_clear();

    if (!was_connected && !ble_gap_adv_active()) {
